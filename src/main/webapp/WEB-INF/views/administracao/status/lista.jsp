...
<my:template fluido="true"
	title="${mensagens.get('StatusListaTituloDaPagina').valor}"
				...
				<div class="card bg-secondary shadow">
					<div class="card-header bg-white border-0">
						<div class="row align-items-center">
							<c:url var="url"
								value="<%=ListaDeURLs.FORMULARIO_INSERCAO_DE_STATUS%>" />
					...
					<div class="card-body">
						<form>
							<c:if test="${empty listaDeStatus}">
								<h6 class="heading-small text-muted mb-4">${mensagens.get('StatusListaSemRegistro').valor}</h6>
							</c:if>
							<div class="pl-lg-4">
								<div class="row">
									...
									<c:if test="${not empty listaDeStatus}">
										<table
											...
											<thead>
												<tr>
													<th>${mensagens.get('StatusListaId').valor}</th>
													<th>${mensagens.get('StatusListaNome').valor}</th>
													<th>${mensagens.get('StatusListaDataIncl').valor}</th>
													<th>${mensagens.get('StatusListaDataAlt').valor}</th>
													<th class="text-center">${mensagens.get('StatusListaAcoes').valor}</th>
												</tr>
											</thead>
												...
												<c:forEach items="${listaDeStatus}" var="status">
													<tr>
														...
														<td><fmt:formatDate pattern="dd/MM/yyyy"
																value="${status.dataIncl.time}" var="data" />${data}</td>
														<td><fmt:formatDate pattern="dd/MM/yyyy"
