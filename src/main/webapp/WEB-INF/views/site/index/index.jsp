			...
			<div class="row">
				...
				<div id="content" class="col-xs-12">
					...
					<div class="slideshow single-slider owl-carousel">
						<div class="item">
							<a href="../site/produtos/listaCategoria/5" title="Camisetas"><img
								class="img-responsive" src="${urlslider}/banner-1.jpg" /></a>
						</div>
						<div class="item">
							<a href="../site/produtos/listaCategoria/9"><img
					...
					<div class="marketshop-banner">
						<div class="row">
							<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
								<a href="../site/produtos/listaCategoria/1"><img
							...
							<div class="owl-carousel product_carousel_tab">
								<c:forEach items="${listaDeCategoria}" var="cat">
									<div class="product-thumb">
										<div class="image">
											<a href="../site/produtos/listaCategoria/${cat.id}"><img
										...
										<a href="../site/produtos/listaCategoria/${cat.id}"
					...
					<div class="category-module" id="latest_category">
						<h3 class="subtitle">
							<i class="fa fa-check-circle"></i> ${Campanha2.nome } - <a
								class="viewall" href="category.tpl">ver todas</a>
						</h3>
						<div class="category-module-content">
							<ul id="sub-cat" class="tabs">
								<c:set var="ref" value="tab-linha" />
								<c:forEach items="${Linha2}" var="linha">
									<li><a href="#${ref}${linha.id}" style="cursor: pointer"
										onclick="carregaProdutoByLinha(${linha.id})">${linha.nome }</a></li>
								</c:forEach>
							</ul>
							<c:forEach items="${Linha2}" var="linhaloop" varStatus="count2">
								<div id="${ref}${linhaloop.id}" class="tab_content">
									<div class="owl-carousel latest_category_tabs">
										<c:set var="i" value="${count2.index}" />
										<c:choose>
											<c:when test="${i == 0}">
												<c:set var="prod" value="${Produto20}" />
											</c:when>
											<c:when test="${i == 1}">
												<c:set var="prod" value="${Produto21}" />
											</c:when>
											<c:when test="${i == 2}">
												<c:set var="prod" value="${Produto22}" />
											</c:when>
											<c:when test="${i == 3}">
												<c:set var="prod" value="${Produto23}" />
											</c:when>
											<c:when test="${i == 4}">
												<c:set var="prod" value="${Produto24}" />
											</c:when>
											<c:when test="${i == 5}">
												<c:set var="prod" value="${Produto25}" />
											</c:when>
											<c:when test="${i == 6}">
												<c:set var="prod" value="${Produto26}" />
											</c:when>
											<c:when test="${i == 7}">
												<c:set var="prod" value="${Produto27}" />
											</c:when>
											<c:when test="${i == 8}">
												<c:set var="prod" value="${Produto28}" />
											</c:when>
											<c:when test="${i == 9}">
												<c:set var="prod" value="${Produto29}" />
											</c:when>
										</c:choose>
