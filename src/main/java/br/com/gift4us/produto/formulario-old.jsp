...
<my:template fluido="false"
	...
	<c:if test="${not empty produto.id}">
		...
		<c:url var="urlimg" value="<%=ListaDeURLs.INSERCAO_DE_PRODUTO%>" />
	</c:if>
			...
			<div class="col-xl-12 order-xl-1">
				<div class="card bg-secondary shadow">
					...
					<form id="ata-cadastro"
							...
							<div class="div-msbc-validator mb-3">
								<label class="col-form-label" for="arquivo">${mensagens.get('ProdutoFormularioImagem').valor}: ${produto.imagem}</label>
								<input class="form-control texto-html"
									...
									<c:if test="${not empty produto.id}">
										<div>
										<a href="${urlimg}"><h4>Outras Imagens</h4></a>
										</div>
									</c:if>
